...
if (isset($_GET['delete_id'])) {
    ...
    $res = $con->query("SELECT file_path FROM notes WHERE id=$delete_id");
    if ($res && $res->num_rows > 0) {
        $row = $res->fetch_assoc();
        if (!empty($row['file_path']) && file_exists($row['file_path'])) {
            unlink($row['file_path']); // server se file delete karega
        }
    }
...
$query = "SELECT id, class, division, subject, file_path, uploaded_on FROM notes WHERE 1=1";
$params = [];
$types = "";
    ...
    <form method="GET" class="row g-3 mb-3">
        <div class="col-md-4">
            <select name="class_filter" class="form-control">
                ...
                <?php
                $classResult = $con->query("SELECT DISTINCT class FROM notes ORDER BY CAST(class AS UNSIGNED)");
        ...
        <div class="col-md-4 d-flex gap-2">
            ...
            <a href="manage-notes.php" class="btn btn-secondary w-100"><i class="bi bi-x-circle"></i> Reset</a>
        </div>
    </form>
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    $fileLink = !empty($row['file_path']) ? "<a href='{$row['file_path']}' target='_blank'>View</a>" : "N/A";
                    echo "<tr>
                        <td>{$sn}</td>
                        <td>{$row['subject']}</td>
                        <td>{$row['class']}</td>
                        ...
                        <td>
                            ...
                            <a href='manage-notes.php?delete_id={$row['id']}' 
